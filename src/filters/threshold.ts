...
export const thresholdFilter: Filter = {
  id: "threshold",
  displayName: "Threshold",
  kind: "fragment",
  wgslSource: `@group(0) @binding(0) var mySampler: sampler;
@group(0) @binding(1) var myTexture: texture_2d<f32>;
@group(0) @binding(2) var<uniform> cutoff: f32;
...
fn fragmentMain(input: VertexOutput) -> @location(0) vec4<f32> {
  ...
  let value = select(0.0, 1.0, luma >= cutoff);
  return vec4<f32>(value, value, value, color.a);
}`,
  uniformParams: [
    {
      name: "cutoff",
